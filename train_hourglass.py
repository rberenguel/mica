#!/usr/bin/env python3
"""
Hourglass folding v2 — phase 1 pre-training on the Gutenberg corpus.
...
Key difference from v1: each HourglassBlock has per-loop LayerNorm parameters,
so shared weights operate in different normalised spaces per iteration.

Saves mica_hourglass_v2_phase1.pt on completion.
Mid-run checkpoint: mica_hourglass_v2_ckpt.pt.
Next: uv run python train_hourglass2.py
"""
import math
import os
import numpy as np
...
from model_hourglass import HourglassTransformer, soft_tie, apply_fold, restore_folds

CHECKPOINT  = "mica_hourglass_v2_ckpt.pt"
WEIGHTS_OUT = "mica_hourglass_v2_phase1.pt"
...
# ── Model ─────────────────────────────────────────────────────────────────────
config = MicaConfig()
print(f"Initializing HourglassTransformer on {config.device.upper()}...")
...
for step in range(start_step, MAX_ITERS):
    ...
    if step % 500 == 0 and step > start_step:
        torch.save({
            ...
            'step':         step,
            'bottom_loops': model.bottom_loops,
            'top_loops':    model.top_loops,
        }, CHECKPOINT)
...
print("Next: uv run python train_hourglass2.py")
