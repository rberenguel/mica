#!/usr/bin/env python3
"""
Phase 2 — fine-tune the small model on noir corpus.

Loads mica_small_n5.pt (Gutenberg pre-trained, unfolded),
fine-tunes on data/train.bin at low LR for ~14K steps.

Usage:
  uv run python src/train_small_noir.py
  uv run python src/train_small_noir.py --lr 5e-5 --steps 14000
"""
import math
import os
import numpy as np
...
PHASE1_CHECKPOINT = "mica_small_n5.pt"
CHECKPOINT        = "mica_small_n5_noir_ckpt.pt"
WEIGHTS_OUT       = "mica_small_n5_noir.pt"
LOG_CSV           = "logs/small/train_log_small_n5_noir.csv"
...
print(f"Initializing small OrigamiTransformer ({config.n_layer} layers, ffn_ratio={config.ffn_ratio}) on {config.device.upper()}...")
...
print(f"Noir tokens: {len(train_data):,} train | {len(val_data):,} val")
...
print(f"Starting noir fine-tune ({max_iters} steps @ LR {max_lr:.0e})...")
...
print(f"\nNoir fine-tune complete. Saved to {WEIGHTS_OUT}  layout={model.layout}")
