# Hourglass v2 — `[2+6+2]` with Loop-Specific LayerNorms

## What changed and why

The first hourglass experiment (`v1`) folded all the way to `[3+4+3]` (6 physical layers, 10 functional passes). Results were underwhelming — the uneven multi-resolution heads were clearly doing good work (visible in the trace UI), but the folding didn't improve quality.
...
| File | State |
|---|---|
| `model_hourglass.py` | **Rewritten** — `HourglassBlock` takes `max_loops`, stores `ln_1`/`ln_2` as `ModuleList`, uses `loop_idx` to index them. All blocks init with `max_loops=3` for headroom (bridge blocks only ever use index 0). |
| `train_hourglass.py` | **Updated** — curriculum stops after first hard fold (`[1+8+1]` → `[2+6+2]`). Total steps: 40,500. Checkpoint names changed to `mica_hourglass_v2_ckpt.pt` / `mica_hourglass_v2_phase1.pt` to avoid accidentally resuming from old v1 checkpoints (state-dict keys are incompatible because LN keys changed from `ln_1.weight` to `ln_1.0.weight`). |
| `train_hourglass2.py` | **Updated** — loads `mica_hourglass_v2_phase1.pt`, outputs `mica_hourglass_v2.pt`. |
| `generate_hourglass.py` | **Updated** — default weights now `mica_hourglass_v2.pt`. Added `--min-new-tokens` (default 5) to prevent immediate EOS termination on terminal prompts. |
...
uv run python train_hourglass.py

# Phase 2 — Noir fine-tuning (standard: 10k steps @ 1e-4)
uv run python train_hourglass2.py

# Phase 2 — Extended fine-tune (20k steps @ 5e-5)
# Use this if the standard phase 2 feels under-cooked.
uv run python train_hourglass2.py
#   → outputs mica_hourglass_v2_long.pt

# Generate
uv run python generate_hourglass.py --weights mica_hourglass_v2_long.pt \
...
If `[2+6+2]` with loop-specific LNs still underperforms the baseline 10-layer model, the hourglass idea itself may not suit this geometry. At that point, the best move is to drop folding entirely and invest the compute budget in:
